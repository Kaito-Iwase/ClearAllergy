Create Codex pet row `running-right` for `kaito-clear`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A small calm 3D-toy/sticker hybrid mascot inspired by allergy safety and focused software work: rounded white capsule body, teal face screen, tiny leaf-check badge, soft coral accent scarf, curious helpful expression, no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
